...
| `--nested-virt` | Enable nested virtualization for the guest (Linux x86_64 only; default off). The guest receives the host's nested CPU capability; guest `/dev/kvm` additionally requires KVM support in the bundled firmware |
