...
- **Masked** — the path is hidden: `lookup` returns `ENOENT`, the name is omitted from `readdir`, and `create`/`rename` into it return `EACCES`. A masked directory that a later rule unmasks a descendant of becomes **traversal-only**: the directory itself is hidden but can still be traversed so the descendant can be reached.
- **TraversalOnly** — hidden itself but traversable for unmasked descendants.

Write admission is enforced separately: a `writes.deny` rule that matches a path blocks writes (`EACCES`) even when the path is otherwise visible. `protect` rules are terminal masks that cannot be overridden by later rules.
...
`rules` are ordered mask/unmask rules (last non-frozen match wins); `protect` are terminal masks; `writes` is an `{allow, deny}` rule set for write admission; `case_sensitivity` is `"sensitive"` (default) or `"insensitive"`. Patterns are mount-root-relative globs; a trailing `/` makes a pattern directory-only.
