...
| Flag | Source | Options |
|------|--------|---------|
| `--mount-dir` | Host directory | `ro`, `rw`, `noexec`, `nosuid`, `nodev`, `stat-virt=strict\|relaxed\|off`, `host-perms=private\|mirror`, `quota=<size>`, `policy=<path>`, paired `uid=<N>,gid=<N>` |
| `--mount-file` | Host file | `ro`, `rw`, `noexec`, `nosuid`, `nodev`, `stat-virt=strict\|relaxed\|off`, `host-perms=private\|mirror`, `quota=<size>`, `policy=<path>`, paired `uid=<N>,gid=<N>` |
...
For file mounts, policy patterns are matched against the HOST file basename (the synthetic root's single entry is evaluated against the host file name), not the guest-visible name.

> For the stack-level story — how the workestrate tooling compiles hierarchical policy config into these program files — see [MOUNT-POLICY.md](https://github.com/rybskiworks/microsandbox/blob/develop/MOUNT-POLICY.md) in this repository (and the workestrate mount-policy concept docs it references).
...
For each guest-visible path the policy returns one of these decisions. On Linux, the backend refines traversal candidates using the actual host object's type without following symlinks:

- **Visible** — the path is exposed normally.
- **Masked** — an untagged host entry is hidden: `lookup`, new read-opens, inode-only `stat`, `unlink`, and renaming it out fail with `ENOENT`, and its name is omitted from new directory listings. Authorized creation over a masked *name* tags the guest-created alias (union semantics); an authorized write-open of a masked host file adopts and tags that host file, making the alias guest-owned. Read visibility through that alias requires the tag to match the opened object's identity. A read-only mount cannot adopt a file this way.
- **TraversalOnly** — a possible ancestor of an unmasked descendant. An actual directory remains discoverable and traversable, with its children filtered independently. A regular file, real symlink (including one pointing to a directory), or file-backed symlink is not a traversal directory: it receives the same read-admission and identity-bound tag checks as **Masked**. The path decision alone never grants a non-directory read access.

For example, masking `**` and then unmasking `**/public` permits traversal through real directories to a file named `public`; it does not expose sibling files merely because the path evaluator conservatively classifies them as possible ancestors.
...
### Admission and runtime upgrades

Read admission is checked before exposing a child name or inode and before issuing a new read handle, inode-only metadata or existence/read/traverse access result, or symlink target. Already-issued file handles retain access to their opened object, including after unlink or replacement; handle-based reads and metadata are not live-revoked. Directory handles retain their point-in-time listing snapshots. Fresh admission still checks the current object, so an old listing cannot authorize reading a denied replacement.

Policies are immutable for a mount's lifetime. Installing a new CLI does not update the filesystem backend inside an already-running runtime. Restart or recreate affected sandboxes with the updated runtime to apply enforcement fixes; this does not require a policy JSON or image-format migration. Rolling back to an older runtime also rolls back its enforcement behavior.
