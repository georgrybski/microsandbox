...
Write admission is enforced separately from visibility: the evaluator takes the union of `writes.allow` and `writes.deny` rules ordered by scope authority, so a higher-authority scope overrides a lower one, with deny rules applied before allow rules within the same scope; the last non-frozen match wins and the default is allow. A rule with `"overridable": false` is terminal and freezes the decision against all later matches. A matching write deny blocks creates, write-opens, attribute changes, and removals with `EACCES` even when the path is otherwise visible; on remove-class operations an untagged masked path still fails with `ENOENT`, since invisibility takes precedence there.
