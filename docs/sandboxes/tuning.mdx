...
Nested virtualization is a creation-time option, outside `modify`. It defaults off: guests receive no nested CPU capability unless you opt in with `--nested-virt` (or `nested_virt` in the sandbox spec / Rust SDK builder). The option presents the host's nested CPU virtualization capability to the guest and is available on Linux x86_64 hosts only. CPU capability alone does not create guest `/dev/kvm`; that additionally requires KVM support in the bundled firmware (libkrunfw `CONFIG_KVM`).
